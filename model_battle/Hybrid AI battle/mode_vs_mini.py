...
###############################################################################
# 3) AIクラスたち (ModelAI, MinimaxAI, HybridBlackAI)
###############################################################################
class ModelAI:
...
def play_one_game(aiA, aiB):
    ...
    # AI名(先手A=黒)
    if isinstance(aiA, HybridBlackAI):
        aiA_name= f"HybridBlackAI(th={aiA.threshold})"
    elif isinstance(aiA, ModelAI):
        aiA_name= f"Model({aiA.model_label})"
    elif isinstance(aiA, MinimaxAI):
        aiA_name= aiA.label
    else:
        aiA_name= "UnknownA"
...
def simulate_games(num_games=5,
                   depthA=3,
                   depthB=3,
                   first_is_minimax=True,
                   first_is_hybrid=False,
                   threshold=15):
    """
    first_is_hybrid=True => 先手(黒)=HybridBlackAI
    そうでなければ first_is_minimaxのTrue/Falseで従来ロジック
    threshold: HybridBlackAIで残り空きマスがこの数以下ならMinimax
    """
    ...
    # (1) モデルファイルを指定
    model_path= "data/model_small"  # or "data/model_medium" / "data/model_large"
    ...
    if first_is_hybrid:
        # ハイブリッド黒AI
        black_model_ai= ModelAI(model_keras, label=loaded_label, my_color=+1)
        black_minimax = MinimaxAI(depth=depthA, my_color=+1)
        aiA= HybridBlackAI(black_model_ai, black_minimax, threshold=threshold)
        black_label= f"HybridBlackAI(th={threshold}, CNN={loaded_label}, Depth={depthA})"
    else:
        if first_is_minimax:
            aiA= MinimaxAI(depth=depthA, my_color=+1)
            black_label= aiA.label
        else:
            aiA= ModelAI(model_keras, label=loaded_label, my_color=+1)
            black_label= f"Model({loaded_label})"

    # (4) 後手(白)AI
    aiB= MinimaxAI(depth=depthB, my_color=-1)
    white_label= aiB.label
    ...
    print(f"黒(先手)勝利: {black_wins}")
    print(f"白(後手)勝利: {white_wins}")
    print(f"引き分け    : {draws}")
...
if __name__=="__main__":
    ...
    # 例: ハイブリッドAI(黒) vs. MinimaxAI(白), 500回対戦
    simulate_games(num_games=500,
                   depthA=5,
                   depthB=5,
                   first_is_minimax=False,
                   first_is_hybrid=True,
                   threshold=15)
